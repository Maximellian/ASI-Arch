...
# Updated batch_train_all.sh: Run ALL experiments found in pool/
...
echo "[$(date)] Starting batch training of all experiments in pool/" | tee "$BATCH_LOG"

# Discover all experiment scripts in pool/ (adjust pattern as needed)
EXPERIMENT_FILES=(pool/*)
EXPERIMENTS=()

for FILE in "${EXPERIMENT_FILES[@]}"; do
  # Extract just the experiment name, removing prefix/suffix
  BASENAME=$(basename "$FILE")
  EXPERIMENT="${BASENAME%.*}" # Remove file extension
...
for i in "${!EXPERIMENTS[@]}"; do
  ...
  echo "$EXPERIMENT,$STATUS,$DURATION,$MODEL_PARAMS,$TOTAL_BENCH,$AVG_TIME,$AVG_TP,$COMPLETION_TIME" >> "$SUMMARY_FILE"
